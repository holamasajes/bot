...
class BaileysProvider extends ProviderClass {
    globalVendorArgs = { name: `api`, gifPlayback: false, usePairingCode: false, phoneNumber: null }
    vendor
    store
            ...
            func: ({ messages, type }) => {
                ...
                if (messageCtx.message?.orderMessage) {
                    ...
                    if (orderId && ordertoken) {
                        ...
                        // (async () => {
                        //     const order = await this.vendor.getOrderDetails(orderId, ordertoken)
                        //     payload = await { ...payload, body: generateRefprovider('_event_order_'), extras: { order } }

                        // })()
                        payload = { ...payload, body: generateRefprovider('_event_order_') };
                    }
                }
     ...
     * @param {any} body
     * @param {any} message
     * @param {any} thumbnailUrl
     * @example await sendCatalog("57xxxxxxxx", "Catalog", "Mira nuestros productos", "Mira todos nuestros productos aqui 👇🏼", "urlImg")
     */
    sendCatalog = async (remoteJid, title, body, message, thumbnailUrl) => {
        const titleC = title || 'Catalogo';
        const bodyC = body || 'Mira nuestros productos';
        const text = message || 'Mira todos nuestros productos aqui 👇🏼';
        const jid = this.vendor?.user?.id;
        let CatalogPath = jid.split(':')[0];
        const productCatalog = thumbnailUrl || 'https://github.com/Ameth1208/PortalQR/blob/main/resources/logo.png?raw=true';

        return await this.vendor?.sendMessage(`${remoteJid}@c.us`, {
            text: text,
            contextInfo: {
                externalAdReply: {
                    title: titleC,
